...
jest.mock("./MagicButton", () => ({
  MagicButton: ({ children, variant }: any) => (
    <button data-testid="magic-button" data-variant={variant}>
      {children}
    </button>
  ),
}));

// Mock RecipeMediaCard so PlannerQueue renders a testable element
jest.mock("./RecipeMediaCard", () => ({
  RecipeMediaCard: ({ title }: any) => (
    <div data-testid="recipe-media-card">{title}</div>
  ),
}));
...
describe("Interactive Tooling - Magic UI Overhaul", () => {
  describe("PantryList", () => {
    it("uses MagicCard for pantry items", () => {
      const items = [
        { id: 1, name: "Apple", category: "Produce", is_low_stock: false },
      ];
      render(
        <PantryList
          items={items}
  ...
  describe("PlannerQueue", () => {
    it("uses RecipeMediaCard for planned recipes", () => {
      const recipes = [
        {
          id: 1,
          title: "Planned Pasta",
      ...
      expect(screen.getByTestId("recipe-media-card")).toBeInTheDocument();
    });
  });
  ...
  describe("ShoppingList", () => {
    it("uses MagicCard as main container", () => {
      ...
      render(
        <ShoppingList items={items} onToggle={() => {}} onRemove={() => {}} />,
      );
      expect(screen.getByTestId("magic-card")).toBeInTheDocument();
    });
  });
